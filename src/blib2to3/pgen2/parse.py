...
class Recorder:
    def __init__(self, parser, ilabels, context):
        ...
        self._points = {
            ilabel: copy.deepcopy(self._start_point)
    ...
    def determine_route(self, force=False):
        alive_ilabels = self.ilabels
        if len(alive_ilabels) == 0:
            last = getattr(self, 'last', None)
            raise NotImplementedError("possibly wrong input: " + repr(last))
...
class Parser(object):
    ...
    def setup(self, proxy, start: Optional[int] = None) -> None:
        """Prepare for parsing.
    ...
    def addtoken(self, type: int, value: Optional[Text], context: Context) -> bool:
        ...
        if len(ilabels) == 1:
            return self._addtoken(*ilabels, type, value, context)
        ...
        with self.proxy.release() as proxy:
            ...
            recorder.last = context
            recorder.add_token(type, value, raw=True)

            while recorder.determine_route() is None:
            ...
            ilabel = recorder.determine_route(force=force)

        return self._addtoken(ilabel, type, value, context)

    def _addtoken(self, ilabel, type: int, value: Optional[Text], context: Context) -> bool:
